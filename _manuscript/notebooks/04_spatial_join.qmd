...
```{r unmatched-parcels}
# Subset parcels NOT in the string-matched set
unmatched_nc1 <- nc1 |>
  filter(!parcel_index %in% stringmatched$parcel_index)

county_list <- unique(unmatched_nc1$cntyfips)

# Filter CL to relevant counties and build point geometry
cl_unmatched <- cl |>
  filter(`FIPS CODE` %in% county_list) |>
  mutate(
    lon = as.numeric(`PARCEL LEVEL LONGITUDE`),
    lat = as.numeric(`PARCEL LEVEL LATITUDE`)
  ) |>
...
unmatched_nc1 <- unmatched_nc1 |>
  mutate(geometry_id = as.integer(factor(as.character(geometry))))

cl_unmatched <- cl_unmatched |>
  mutate(geometry_id1 = as.integer(factor(as.character(geometry))))
```
...
pcl_comb <- bind_rows(cl_string, cl_dist_nosort, cl_dist_sorted) |>
  mutate(geometry_id = replace_na(as.character(geometry_id), "")) |>
