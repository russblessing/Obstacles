...
reassign_nohit <- function(direct_df, addr_col, id_col,
  ...
  nn <- nngeo::st_nn(nohit, high, k = k, returnDist = TRUE, progress = FALSE)

  # Long-format: one row per (input record × K candidates)
  cand <- tibble(
    !!id_col       := rep(nohit[[id_col]], each = k),
    input_addr     = rep(nohit[[addr_col]], each = k),
    cand_pos       = rep(seq_len(k), nrow(nohit)),
    cand_row       = unlist(nn$nn),
    cand_dist_m    = unlist(nn$dist)
  ) |>
    mutate(
