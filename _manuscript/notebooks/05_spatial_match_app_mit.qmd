...
to_unique_geoms <- function(df) {
  ...
  df |>
    st_transform(crs = 32119) |>
    (\(x) mutate(x, geom_str   = as.character(st_geometry(x)),
                    geometry_id = as.integer(factor(geom_str))))() |>
    distinct(geom_str, .keep_all = TRUE) |>
    select(geometry_id, geom_str, all_of(geom_col)) |>
...
ug_med  <- to_unique_geoms(medpri) |>
  filter(!geom_str %in% ug_high$geom_str)

ug_low  <- to_unique_geoms(lowpri) |>
  filter(!geom_str %in% ug_high$geom_str,
         !geom_str %in% ug_med$geom_str)

c(nrow(ug_high), nrow(ug_med), nrow(ug_low))
```
