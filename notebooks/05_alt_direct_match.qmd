...
# Both are set to "unique_id" which is assigned in the canonical Step 5.
...
mits <- st_read(mits_path, quiet = TRUE) |> st_transform(st_crs(parcels))
apps <- st_read(apps_path, quiet = TRUE) |> st_transform(st_crs(parcels))
```
...
- **Input paths** default to `mit_sa_full_noqa.gpkg` / `app_sa_full_noqa.gpkg`
  (the pre-QA intermediates written by the canonical Step 5). Confirm these are
  the right inputs, or swap in the raw study-area files before QA is applied.
