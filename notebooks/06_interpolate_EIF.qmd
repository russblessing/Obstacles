...
split_cols <- function(df) {
  ...
  val_pat  <- "count|value|n_raw|n_pp|pop|hu_|_raw|_pp|estimate|moe"
...
aggregate_eif_wide <- function(d, prefix) {
  cols <- split_cols(d)
  if (length(cols$key) == 0 || length(cols$val) == 0) {
    # Already one row per cell, or no recognisable value columns — just prefix.
    d |>
      select(eif_lat, eif_lon, any_of(cols$val)) |>
      rename_with(~ paste0(prefix, "_", .), -c(eif_lat, eif_lon))
  } else {
    d |>
      pivot_wider(
        id_cols     = c(eif_lat, eif_lon),
        names_from  = all_of(cols$key),
        values_from = all_of(cols$val),
        names_sep   = "_",
        values_fn   = sum        # handles any residual duplication
      ) |>
      rename_with(~ paste0(prefix, "_", .), -c(eif_lat, eif_lon))
  }
}
