...
# Pre-build NC-filtered tables ONCE per dataset using Arrow pushdown —
# avoids loading the full national parquet into memory multiple times.
# Each table is kept as a plain data frame; sf conversion for the
# nearest-neighbour search is done on the keys-only subset.

eif_nc_tables <- map(names(eif_files), function(nm) {
  f <- eif_files[[nm]]
...
# Keys-only sf layers for nearest-neighbour search (tiny memory footprint)
...
cat("NC EIF cell counts per dataset:\n")
...
```{r eif-join}
# Reuse the pre-built NC tables — no further collect() calls needed.
