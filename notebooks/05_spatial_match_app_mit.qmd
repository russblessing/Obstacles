...
mit_path            <- "/proj/mhinolab/projects/obstacles2/CRC_May_2025/mit_newgeo.gpkg"
mit_elev_path       <- "/proj/mhinolab/projects/obstacles2/CRC_May_2025/mit_elevation_qaqc_filled.csv"
app_path            <- "/proj/mhinolab/projects/obstacles2/CRC_May_2025/applications_final.gpkg"
...
```{r load-parcels}
# Parcel–CL merge from Step 4 (all character to preserve leading zeros, etc.)
...
# Parcel geometries with geometry_id (factorized unique geometry key)
nc1_geoms <- st_read(geoms_path, quiet = TRUE) |>
  mutate(
    parcel_index = as.integer(parcel_index),
    geometry_id  = as.integer(factor(as.character(st_geometry(pick(everything())))))
  ) |>
  select(parcel_index, geometry_id)

# Join geometries back to parcel attributes
parcels <- parcels_raw |>
  # drop any stale geometry columns carried over from the CSV
...
mit_elev <- read_csv(mit_elev_path, col_types = cols(.default = col_character())) |>
  rename(elevation_score = `elevation_score `)   # remove trailing-space typo

mit_gdf <- mit_gdf |>
  left_join(mit_elev |> select(record_ID, elevation_score), by = "record_ID")

# Clip to study area
mit_gdf <- mit_gdf |>
  mutate(Inside_SA = as.integer(lengths(st_within(geometry, flood_boundary)) > 0))
...
app <- st_read(app_path, quiet = TRUE) |>
  ...
  mutate(
    app_id    = row_number(),
    Inside_SA = as.integer(lengths(st_within(geometry, flood_boundary)) > 0)
  )
...
to_unique_geoms <- function(df, id_col = "geometry_id") {
  df |>
    st_transform(crs = 32119) |>
    distinct(across(all_of(id_col)), .keep_all = TRUE) |>
    select(all_of(id_col), geometry = geom)
}
...
```{r unique-geoms-mit-app}
# Assign a group indicator for unique geometries in mit/app
mit_sa <- mit_sa |>
  mutate(g_indicator = as.integer(factor(as.character(st_geometry(pick(everything()))))))

app_sa <- app_sa |>
  mutate(
    g_indicator = as.integer(factor(as.character(st_geometry(pick(everything))))),
    app_id      = row_number()
  )

unique_mit <- mit_sa |> distinct(g_indicator, .keep_all = TRUE) |>
  select(g_indicator, geometry) |>
  st_transform(crs = st_crs(flood_boundary))
...
match_to_tier <- function(candidates, parcel_tier, max_dist = 50) {
  ...
  unmatched <- intersect_raw |>
    filter(is.na(geometry_id)) |>
    select(g_indicator, geometry)           # drop stale geometry_id col
  ...
  if (nrow(unmatched) == 0 || nrow(parcel_tier) == 0) {
    ...
    dist_raw <- st_join(unmatched, parcel_tier, join = st_nearest_feature) |>
      mutate(distance = as.numeric(
        st_distance(geometry,
                    parcel_tier$geometry[match(geometry_id, parcel_tier$geometry_id)],
                    by_element = TRUE)
      )) |>
      st_drop_geometry() |>
      distinct(g_indicator, .keep_all = TRUE) |>
    ...
    dist_matched <- dist_raw
  }

  # Combine and return only the key columns
  bind_rows(
...
nn_fallback <- function(remaining, all_geoms) {
  ...
  st_join(remaining, all_geoms, join = st_nearest_feature) |>
    mutate(distance = as.numeric(
      st_distance(geometry,
