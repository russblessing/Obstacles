...
get_race_med <- function(panel_label, stage_name) {
  v <- inputs$fig4_medians |>
    filter(race == panel_label, stage == stage_name) |>   # was `panel ==`
    pull(median_pct)
  round(v, 1)
}
...
![Overview of the flooded-to-funded mitigation pipeline and the key datasets contributing to each stage.](figures/Fig5_overview.png.png){#fig-overview fig-alt="Diagram showing five pipeline stages — flooded, eligible, in an applying community, applied, funded — with the data source linked to each stage" width=100%}
