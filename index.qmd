...
# Discussion
Our analysis identifies the application stage as the critical pinch point in the flooded-to-funded HMA pipeline. Of all flooded residential properties in eastern North Carolina, only `r pct_applied_of_flood`% themselves apply for funding, and of those, `r pct_funded_of_applied`% receive an award. By tracking the full population of flooded properties rather than only those that received funding, we identify a substantial population of households who would benefit from mitigation assistance but who do not enter the applicant pool. This unrealized demand, to our knowledge not previously quantified at the property scale, suggests that interventions aimed at increasing application rates among flooded households may have larger returns than further refinement of selection criteria among existing applicants.

Among the small fraction of properties that do receive funding, the composition shifts toward less expensive properties and lower-value neighborhoods. Funded properties sit at the `r med_A_funded`th percentile of the study-area property-value distribution, compared to the `r med_A_flood`th percentile of all flooded properties. The shift occurs primarily between neighborhoods rather than within them: within-block-group percentiles for funded properties differ only modestly from those at the flooded stage, indicating that funded properties tend to be typical homes in lower-value neighborhoods rather than the cheaper homes within any given neighborhood. This is consistent with prior county-scale evidence that within HMA-participating counties, lower-income communities are disproportionately the locations of buyout and elevation activity [@machManagedRetreatVoluntary2019; @elliottRacialInequitiesFederal2020a]. The neighborhood-scale selection we observe could reflect several mechanisms: cost-benefit analysis criteria that favor lower-cost properties, programmatic concentration of buyouts to enable contiguous open-space conversion, or differential outreach by local governments to particular neighborhoods. Disentangling these is an important direction for future work.

Neighborhood racial composition shifts in the same direction across the pipeline. Block groups containing funded properties have a median non-Hispanic white share of `r med_white_funded`%, compared to `r med_white_flood`% at the flooded stage; Black share rises correspondingly from `r med_black_flood`% to `r med_black_funded`%. The direction and magnitude of this shift align with prior county-scale analyses [@machManagedRetreatVoluntary2019; @elliottRacialInequitiesFederal2020a], but the parcel-level resolution localizes the selection to the neighborhood scale rather than the individual-property scale. The wealth and racial shifts appear to be two views of the same neighborhood-level selection process. Funding flows to lower-value, less-white neighborhoods, but within those neighborhoods, no particular type of home is preferentially funded. This replicates at parcel resolution what the prior literature has documented at the county scale. This pattern likely reflects the strong correlation between race and income in eastern North Carolina, where less-white neighborhoods tend to also be lower-value. The neighborhood-scale wealth selection described above produces a racial shift through this correlation, suggesting that the racial composition pattern may not represent independent demographic targeting but rather a consequence of where lower-value flood-exposed neighborhoods happen to be located.

While our findings provide a critical first assessment of the application process, several questions and limitations remain. First, our flood-exposure data come from the FLDEX archive [@garciaReconstructingRepetitiveFlood2025], which captures 78 discrete flood events in eastern North Carolina between 1996 and 2020. The underlying models are conservative and likely underestimate the true population of flood-exposed properties. Second, our data on the timing of applications and mitigations is incomplete, so we do not distinguish between floods that occur before or after a property is funded for mitigation. Third, we have not decomposed funded properties by mitigation type (buyouts versus elevations), which are methodologically distinct: buyout funding requires household relocation, while elevation preserves in-place residence. The wealth and racial-composition shifts we observe may differ meaningfully between these mitigation types, and this analysis is a priority for future work. Fourth, our block-group demographic data come from the 2013 American Community Survey, which falls near the midpoint of the study window but does not capture demographic changes over the full 1996–2020 period. Finally, while we document more of the application process than past studies, there are still many aspects that we do not observe — such as how local governments approach (or fail to approach) flooded households about funding opportunities, procedural obstacles that may deter applications from interested households, and sources of non-governmental assistance.

## Conclusions
Our paper is the first to follow individual residential properties from flood exposure through mitigation funding outcome at each significant stage, including the previously-unobservable application stage. The largest falloff in the pipeline is the application step, where the composition of properties that continue along the pipeline shifts toward lower-value properties and less-white neighborhoods, primarily through between-neighborhood selection rather than within-neighborhood selection. These results indicate substantial potential to improve the rate of applications from flood-prone households. At the same time, given that only `r pct_funded_of_applied`% of applications receive funding, increases in the amount of public assistance for mitigation are necessary to address existing demand, much of which originates from marginalized populations. These lessons from the North Carolina context inform future research seeking to link flooding to funding outcomes and address equity considerations along the way.
...
## Pipeline analysis
Property values are taken from the most recent CoreLogic vintage and used at the parcel level. To make values comparable across counties with different assessment cycles and price levels, we express each property's value as a percentile rank across all 1–4 family residential parcels in the study area, and use this study-area-wide percentile rank as our primary property-value measure throughout the analysis. To complement the broad ranking, we also compute each property's value as a percentile rank within its own census block group. This within-neighborhood percentile lets us test whether selection toward lower-value properties occurs within neighborhoods or primarily reflects flow toward lower-value neighborhoods.
...
The five-stage pipeline is a sequence of nested subsets: each stage is, by construction, a subset of the previous one. We characterize each stage in three ways. First, we count the number of parcels that survive into the stage and report stage-to-stage attrition rates, separately for once-flooded and repeatedly flooded parcels. Second, we describe the distribution of flood-exposure intensity (count of intersecting events) at each stage, to test whether the parcels surviving deeper into the pipeline are systematically those with more severe flood histories. Third, we describe the distribution of property values (study area wide percentile rank) and block-group demographics at each stage, to identify where in the pipeline shifts in wealth and racial composition emerge.
...
![](figures/fig5a_income.png){#fig-income width=100%}


![](figures/fig5b_home_value.png){#fig-homevalue width=100%}
