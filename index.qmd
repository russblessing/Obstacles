...
While most flooded properties (`r pct_comm_of_flood`%) are in communities that have applied for funding, only `r pct_applied_of_flood`% submit an application (@fig-funnel). Among all flooded properties (@fig-funnel, left panel), `r pct_comm_of_flood`% are in jurisdictions where the local government submitted at least one HMA application. Only `r pct_applied_of_flood`% of flooded properties apply, and `r pct_funded_of_applied`% of those are funded (only `r pct_funded_of_flood`% of all flooded properties).
